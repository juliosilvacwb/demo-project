...
test.describe("Workouts", () => {
  ...
  test.beforeEach(async ({ page }) => {
    email = `test-workouts-${Date.now()}-${Math.random().toString(36).slice(2, 7)}@example.com`;
  ...
  test.describe("progression chart", () => {
    test("should display the progression chart when multiple workouts exist", async ({ page }) => {
      await page.goto("/movements");
      await page.waitForLoadState("networkidle");
      const movementName = `Chart Movement ${Math.random().toString(36).slice(2, 7)}`;
      await page.fill('input[placeholder="Movement name (e.g. Bench Press)"]', movementName);
      await page.click('button:has-text("Add")');
      await expect(page.locator('input[placeholder="Movement name (e.g. Bench Press)"]')).toHaveValue("");
    ...
    test("should show empty state when only one workout exists", async ({ page }) => {
      await page.goto("/movements");
      await page.waitForLoadState("networkidle");
      const movementName = `Empty Chart ${Math.random().toString(36).slice(2, 7)}`;
      await page.fill('input[placeholder="Movement name (e.g. Bench Press)"]', movementName);
      await page.click('button:has-text("Add")');
      await expect(page.locator('input[placeholder="Movement name (e.g. Bench Press)"]')).toHaveValue("");
